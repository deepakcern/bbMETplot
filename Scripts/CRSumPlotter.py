...
c=TCanvas()
c.SetLogy()
labels=['1e2b','2e2b','1mu2b','2mu2b']#,'1e1mu2b']
...
path='/Users/dekumar/cernbox/monoH_work/plottingAndCombinedRoot_work/CRSumFiles/*.root'
...
for file in files:
    ...
    if 'Ele' in file.split('/')[-1]:
        ...
        for i in range(len(gotHists_e)):
            h_data.SetBinContent(1, h_mydata.GetBinContent(2))
            h_data.SetBinContent(2, h_mydata.GetBinContent(4))

        for i in range(len(gotHists_e)):
            legend.AddEntry(myHists[i],leg_entry[i],"F")
            myHists[i].SetFillColor(colors[i])
            myHists[i].SetMaximum(1000000)
            myHists[i].SetBinContent(1, gotHists_e[i].GetBinContent(2))
            myHists[i].GetXaxis().SetBinLabel(1, labels[0])
            myHists[i].SetBinContent(2, gotHists_e[i].GetBinContent(4))
            myHists[i].GetXaxis().SetBinLabel(2, labels[1])

        #fill the histograms
    elif 'Mu' in file.split('/')[-1]:
        ...
        h_data.SetBinContent(3, h_mydata.GetBinContent(2))
        h_data.SetBinContent(4, h_mydata.GetBinContent(4))
        # h_data.SetBinContent(5, h_mydata.GetBinContent(6))
        for i in range(len(gotHists_mu)):
                myHists[i].SetBinContent(3, gotHists_mu[i].GetBinContent(2))
                ...
                # myHists[i].SetBinContent(5, gotHists_mu[i].GetBinContent(6))
