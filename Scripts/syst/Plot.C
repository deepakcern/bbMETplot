...
TString dirpathname;
 ...
 dirpathname = "04072018"; //.Form("%d%1.2d%d",ltm->tm_mday,1 + ltm->tm_mon,1900 + ltm->tm_year);
 ...
 ofstream mout;
 mout.open(DirPreName+dirpathname +"/reg_1e2b_ewkTop_syst_down"+dirpathname +"Integral.txt",std::ios::app);
 ofstream rout;
 rout.open(DirPreName+dirpathname +"/reg_1e2b_ewkTop_syst_down"+dirpathname +"Integral.html",std::ios::app);
 ofstream tableout;
 tableout.open(DirPreName+dirpathname +"/reg_1e2b_ewkTop_syst_down"+dirpathname +"IntegralWithError.txt",std::ios::app);
 TString outputshapefilename = DirPreName+dirpathname +"/reg_1e2b_ewkTop_syst_down.root";
...
ofstream metbinsout_3;
 ...
 metbinsout_1.open(DirPreName+"METBIN_1/reg_1e2b_ewkTop_syst_down"+dirpathname +"Integral.txt",std::ios::app);
 metbinsout_2.open(DirPreName+"METBIN_2/reg_1e2b_ewkTop_syst_down"+dirpathname +"Integral.txt",std::ios::app);
 metbinsout_3.open(DirPreName+"METBIN_3/reg_1e2b_ewkTop_syst_down"+dirpathname +"Integral.txt",std::ios::app);
}
...
filenameString.push_back(filenamepath + "Output_crab_QCD_HT50to100_TuneCUETP8M1_13TeV-madgraphMLM-pythia8.root");
filenameString.push_back(filenamepath + "Output_crab_QCD_HT100to200_TuneCUETP8M1_13TeV-madgraphMLM-pythia8.root");
filenameString.push_back(filenamepath + "Output_crab_QCD_HT200to300_TuneCUETP8M1_13TeV-madgraphMLM-pythia8.root");
filenameString.push_back(filenamepath + "Output_crab_QCD_HT300to500_TuneCUETP8M1_13TeV-madgraphMLM-pythia8.root");
...
//Data File 84
filenameString.push_back(filenamedatapath + "data_combined_SE.root");
...
TString histnameString("h_ewkTop_syst_1e2b_down_");
...
Xsec[39] = 0;    //1712000 * 0.9458;                // QCD_HT200to300
Xsec[40] = 0;    //347700 * 0.9458;                 // QCD_HT300to500
Xsec[41] = 0*32100 * 0.9458;                  // QCD_HT500to700
Xsec[42] = 0*6831 * 0.9458;                   // QCD_HT700to1000
Xsec[43] = 0*1207 * 0.9458;                   // QCD_HT1000to1500
Xsec[44] = 0*119.9 * 0.9458;                   // QCD_HT1500to2000
Xsec[45] = 0*25.24 * 0.9458;                   // QCD_HT2000toInf
...
//    cout << "Reading file #" << to_string(i+1) << ": " << filenameString[i] << endl;
    ...
    if(h_total->Integral()>0) normalization[i]     = (lumi* Xsec[i])/(h_total->Integral()*1);
...
if (Stackhist->GetEntries()==0){
    ...
    empfile << "h_ewkTop_syst_1e2b_down_" <<endl;
    empfile.close();
}
...
TH1F* h_prefit;
TFile* fprefit;
TString prefitfilename = "PREFITDIR/reg_1e2b_ewkTop_syst_down.root";
if(0){
...
//  }
  if(!0)
  {
      if(1==1)    hs->SetMinimum(0.68);
      if(1==0)   hs->SetMinimum(0);
      if(1==0) hs->SetMaximum(maxi *1.4);
  }
  else   {
      if(1==1)    hs->SetMinimum(0.68);
      if(1==0)   hs->SetMinimum(0);
...
//hs->GetXaxis()->SetTickLength(0.07);
  ...
  //hs->GetXaxis()->SetTitle("Hadronic Recoil (GeV)");
  }
  else{
  //hs->GetXaxis()->SetTitle("Hadronic Recoil (GeV)");
...
//  DataMCPre->Divide(h_prefit);
  ...
  DataMC->GetXaxis()->SetTitle("Hadronic Recoil (GeV)");
...
float zjets_error = TMath::Sqrt(pow(Integral_Error[3],2) + pow( Integral_Error[4],2) + pow(Integral_Error[5],2) + pow(Integral_Error[6],2) + pow(Integral_Error[7],2) + pow(Integral_Error[8],2)+ pow(Integral_Error[9],2)+ pow(Integral_Error[10],2));
  ...
  mout << "reg_1e2b_ewkTop_syst_down"            <<  " a b"<<std::endl;
...
if(1==0){
// c12->SaveAs(DirPreName+dirpathname +"/bbMETPdf/reg_1e2b_ewkTop_syst_down.pdf");
//c12->SaveAs(DirPreName+dirpathname +"/bbMETPng/reg_1e2b_ewkTop_syst_down.png");
// cout << "Saved." << endl;
c12->SaveAs(DirPreName+dirpathname +"/bbMETROOT/reg_1e2b_ewkTop_syst_down.root");
 rout<<"<hr/>"<<std::endl;
 rout<<"<table class=\"\"> <tr><td><img src=\""<<"DYPng/reg_1e2b_ewkTop_syst_down.png\" height=\"400\" width=\"400\"></td>   </tr> </table>"<<std::endl;

}

if(1==1){
// c12->SaveAs(DirPreName+dirpathname +"/bbMETPdf/reg_1e2b_ewkTop_syst_down_log.pdf");
// c12->SaveAs(DirPreName+dirpathname +"/bbMETPng/reg_1e2b_ewkTop_syst_down_log.png");
 cout << "Saved." << endl;
c12->SaveAs(DirPreName+dirpathname +"/bbMETROOT/reg_1e2b_ewkTop_syst_down_log.root");
}
