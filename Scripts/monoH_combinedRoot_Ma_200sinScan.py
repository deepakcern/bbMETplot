...
os.system('mkdir DataCardRootFiles_sin_ma200')
...
f=TFile("DataCardRootFiles_sin_ma200/AllMETHistos.root","RECREATE")
f.cd()
...
MH4_200_CS=[0.03089,0.1106,0.2051,0.2742,0.2922,0.2512,0.1848]
...
for inFile in HDMaFiles:
    ...
    if 'MH4_200' not in inFile.split('/')[-1]: continue

    print ("Total = "+str(tot))

    for i in range(len(MH4_200_CS)):
        if '2HDMa' in inFile:
        ...
        h_temp=setHistStyle(h_temp2,bins,newname)
        sel=h_temp.Integral()
        h_temp.Scale(lumi*(MH4_200_CS[i])/tot)
        f.cd()
        h_temp.Write()
